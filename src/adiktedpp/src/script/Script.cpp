...
namespace adiktedpp {
    namespace script {
        ...
        std::string script_keyword( const Timer data ) {
            switch( data ) {
            case Timer::T_TIMER_0: { return "TIMER0"; }
            case Timer::T_TIMER_1: { return "TIMER1"; }
            case Timer::T_TIMER_2: { return "TIMER2"; }
            case Timer::T_TIMER_3: { return "TIMER3"; }
            case Timer::T_TIMER_4: { return "TIMER4"; }
            case Timer::T_TIMER_5: { return "TIMER5"; }
            case Timer::T_TIMER_6: { return "TIMER6"; }
            case Timer::T_TIMER_7: { return "TIMER7"; }
            }
        ...
        void ScriptCommand::RANDOMISE_FLAG( const adiktedpp::Player player, const Flag flag, const int maxValue ) {
            std::stringstream stream;
            stream << "RANDOMISE_FLAG( " << script_keyword( player ) << ", " << script_keyword( flag ) << ", " << maxValue << " )";
            addLine( stream.str() );
        }

        void ScriptCommand::SET_TIMER( const adiktedpp::Player player, const Timer timer ) {
            std::stringstream stream;
            stream << "SET_TIMER( " << script_keyword( player ) << ", " << script_keyword( timer ) << " )";
            addLine( stream.str() );
        }
...
//            stream << "DUNGEON_HEART" << ", ";
        ...
        void ScriptCommand::NEXT_COMMAND_REUSABLE() {
            addLine( "NEXT_COMMAND_REUSABLE" );
        }
        ...
        const std::set< ScriptSection >& AllScriptSections() {
            ...
            if ( data.empty() ) {
                ...
                data.insert( ScriptSection::SS_FLAGS );
...
//                currSection.copyIndexes( fromSection );
        ...
        void Script::setCreatureAvailable( const Player player, const Creature item, const bool available, const int numberForced ) {
            /**
             * old interface:
             ...
             */
            std::stringstream stream;
            stream << AvailableCommand::AC_CREATURE_AVAILABLE << "( " << script_keyword( player ) << ", " << script_keyword( item ) << ", " << (int)available << ", " << numberForced << " )";
            const std::string& line = stream.str();
            init.addLine( line );
        }

        void Script::setTrapAvailable( const Player player, const Trap item, const bool constructable, const int numberAvailable ) {
            std::stringstream stream;
            stream << AvailableCommand::AC_TRAP_AVAILABLE << "( " << script_keyword( player ) << ", " << script_keyword( item ) << ", " << (int)constructable << ", " << numberAvailable << " )";
            const std::string& line = stream.str();
            init.addLine( line );
        }

        void Script::setMagicAvailable( const Player player, const Spell item, const MagicAvailableMode mode ) {
            switch( mode ) {
            case MagicAvailableMode::AM_DISABLED: {
                std::stringstream stream;
                stream << AvailableCommand::AC_MAGIC_AVAILABLE << "( " << script_keyword( player ) << ", " << script_keyword( item ) << ", 0, 0 )";
                const std::string& line = stream.str();
                init.addLine( line );
                break;
            }
            case MagicAvailableMode::AM_RESEARCHABLE: {
                std::stringstream stream;
                stream << AvailableCommand::AC_MAGIC_AVAILABLE << "( " << script_keyword( player ) << ", " << script_keyword( item ) << ", 1, 0 )";
                const std::string& line = stream.str();
                init.addLine( line );
                break;
            }
            case MagicAvailableMode::AM_AVAILABLE: {
                std::stringstream stream;
                stream << AvailableCommand::AC_MAGIC_AVAILABLE << "( " << script_keyword( player ) << ", " << script_keyword( item ) << ", 1, 1 )";
                const std::string& line = stream.str();
                init.addLine( line );
                break;
            }
        ...
        void Script::set( const RoomsAvailableState& stateMap ) {
            ...
            for ( ; iter1 != stateMap.end(); ++iter1 ) {
                ...
                for ( ; iter2 != submap.end(); ++iter2 ) {
                    ...
                    setRoomAvailable( player, item, state );
                }
            }
        }
        ...
        void Script::set( const CreatureAvailableState& stateMap ) {
            ...
            for ( ; iter1 != stateMap.end(); ++iter1 ) {
                ...
                for ( ; iter2 != submap.end(); ++iter2 ) {
                    const auto item   = iter2->first;
                    const auto& state = iter2->second;
                    setCreatureAvailable( player, item, state.available, state.numberForced );
                }
            }
        }
        ...
        void Script::set( const DoorAvailableState& stateMap ) {
            ...
            for ( ; iter1 != stateMap.end(); ++iter1 ) {
                ...
                for ( ; iter2 != submap.end(); ++iter2 ) {
                    const auto item   = iter2->first;
                    const auto& state = iter2->second;
                    setDoorAvailable( player, item, state.constructable, state.numberAvailable );
                }
            }
        }
        ...
        void Script::set( const TrapAvailableState& stateMap ) {
            ...
            for ( ; iter1 != stateMap.end(); ++iter1 ) {
                ...
                for ( ; iter2 != submap.end(); ++iter2 ) {
                    const auto item   = iter2->first;
                    const auto& state = iter2->second;
                    setTrapAvailable( player, item, state.constructable, state.numberAvailable );
                }
            }
        }
        ...
        void Script::set( const MagicAvailableState& stateMap ) {
            ...
            for ( ; iter1 != stateMap.end(); ++iter1 ) {
                ...
                for ( ; iter2 != submap.end(); ++iter2 ) {
                    const auto item   = iter2->first;
                    const auto& state = iter2->second;
                    setMagicAvailable( player, item, state );
                }
            }
        }
        ...
        void Script::setRoomsAvailable( const Player player, const RoomAvailableMode mode ) {
            RoomsAvailableState availableState;
            availableState.setAllAvailable( player, mode );
            set( availableState );
        }

        void Script::setRoomAvailable( const Player player, const Room room, const RoomAvailableMode mode ) {
            switch( mode ) {
            case RoomAvailableMode::RAM_DISABLED: {
                std::stringstream stream;
                stream << AvailableCommand::AC_ROOM_AVAILABLE << "( " << script_keyword( player ) << ", " << script_keyword( room ) << ", 0, 0 )";
                const std::string& line = stream.str();
                init.addLine( line );
                break ;
            }
            case RoomAvailableMode::RAM_RESEARCHABLE_FOUND: {
                std::stringstream stream;
                stream << AvailableCommand::AC_ROOM_AVAILABLE << "( " << script_keyword( player ) << ", " << script_keyword( room ) << ", 4, 0 )";
                const std::string& line = stream.str();
                init.addLine( line );
                break ;
            }
            case RoomAvailableMode::RAM_RESEARCHABLE: {
                std::stringstream stream;
                stream << AvailableCommand::AC_ROOM_AVAILABLE << "( " << script_keyword( player ) << ", " << script_keyword( room ) << ", 1, 0 )";
                const std::string& line = stream.str();
                init.addLine( line );
                break ;
            }
            case RoomAvailableMode::RAM_AVAILABLE_FOUND: {
                std::stringstream stream;
                stream << AvailableCommand::AC_ROOM_AVAILABLE << "( " << script_keyword( player ) << ", " << script_keyword( room ) << ", 3, 0 )";
                const std::string& line = stream.str();
                init.addLine( line );
                break ;
            }
            case RoomAvailableMode::RAM_AVAILABLE: {
                std::stringstream stream;
                stream << AvailableCommand::AC_ROOM_AVAILABLE << "( " << script_keyword( player ) << ", " << script_keyword( room ) << ", 1, 1 )";
                const std::string& line = stream.str();
                init.addLine( line );
                break ;
            }
        ...
        void Script::setDoorAvailable( const Player player, const Door item, const bool constructable, const int numberAvailable ) {
            std::stringstream stream;
            stream << AvailableCommand::AC_DOOR_AVAILABLE << "( " << script_keyword( player ) << ", " << script_keyword( item ) << ", " << (int)constructable << ", " << numberAvailable << " )";
            const std::string& line = stream.str();
            init.addLine( line );
        }

        void Script::setDoorsAvailable( const Player player, const int numberAvailable ) {
            DoorAvailableState availableState;
            availableState.setAllAvailableAmount( player, numberAvailable );
            set( availableState );
        }

        void Script::setTrapsAvailable( const Player player, const int numberAvailable ) {
            TrapAvailableState availableState;
            availableState.setAllAvailableAmount( player, numberAvailable );
            set( availableState );
        }

        void Script::setMagicAvailable( const Player player, const MagicAvailableMode mode ) {
        ...
        void Script::setPrisonConvertLimits() {
            main.addLineIndent( "rem - prevent prison convert snow-balling -" );
            main.addLineIndent( "IF( PLAYER0, SKELETON > 2 )" );
            main.addLineIndent( "    SET_GAME_RULE( PrisonSkeletonChance, 80 )" );
            main.ADD_TO_FLAG( Player::P_P0, Flag::F_FLAG_7, 1 );
            main.addLineIndent( "ENDIF" );
            main.addLineIndent( "IF( PLAYER0,SKELETON > 5 )" );
            main.addLineIndent( "    SET_GAME_RULE( PrisonSkeletonChance, 60 )" );
            main.ADD_TO_FLAG( Player::P_P0, Flag::F_FLAG_7, 1 );
            main.addLineIndent( "ENDIF" );
            main.addLineIndent( "IF( PLAYER0,SKELETON > 8 )" );
            main.addLineIndent( "    SET_GAME_RULE( PrisonSkeletonChance, 40 )" );
            main.ADD_TO_FLAG( Player::P_P0, Flag::F_FLAG_7, 1 );
            main.addLineIndent( "ENDIF" );
            main.addLineIndent( "IF( PLAYER0,GHOST > 3 )" );
            main.addLineIndent( "    SET_GAME_RULE( GhostConvertChance, 30 )" );
            main.ADD_TO_FLAG( Player::P_P0, Flag::F_FLAG_7, 1 );
            main.addLineIndent( "ENDIF" );
        }

        void Script::setTortureConvertLimits() {
            main.addLineIndent( "rem - prevent torture convert snow-balling -" );
            main.addLineIndent( "IF( PLAYER0, CREATURES_CONVERTED > 2 )" );
            main.addLineIndent( "    SET_GAME_RULE( TortureDeathChance, 33 )" );
            main.ADD_TO_FLAG( Player::P_P0, Flag::F_FLAG_7, 1 );
            main.addLineIndent( "ENDIF" );
            main.addLineIndent( "IF( PLAYER0, CREATURES_CONVERTED > 5 )" );
            main.addLineIndent( "    SET_GAME_RULE( TortureDeathChance, 50 )" );
            main.ADD_TO_FLAG( Player::P_P0, Flag::F_FLAG_7, 1 );
            main.addLineIndent( "ENDIF" );
            main.addLineIndent( "IF( PLAYER0,CREATURES_CONVERTED > 8 )" );
            main.addLineIndent( "    SET_GAME_RULE( TortureDeathChance, 80 )" );
            main.ADD_TO_FLAG( Player::P_P0, Flag::F_FLAG_7, 1 );
            main.addLineIndent( "ENDIF" );
        }

        void Script::setStunChance() {
            main.addLineIndent( "rem - modify stun chance -" );
            main.addLineIndent( "IF( PLAYER0,FLAG7 >= 1 )" );
            main.addLineIndent( "    SET_GAME_RULE( StunEvilEnemyChance, 80 )" );
            main.addLineIndent( "    SET_GAME_RULE( StunGoodEnemyChance, 80 )" );
            main.addLineIndent( "ENDIF" );
            main.addLineIndent( "IF( PLAYER0,FLAG7 >= 2 )" );
            main.addLineIndent( "    SET_GAME_RULE( StunGoodEnemyChance, 60 )" );
            main.addLineIndent( "ENDIF" );
            main.addLineIndent( "IF( PLAYER0,FLAG7 >= 3 )" );
            main.addLineIndent( "    SET_GAME_RULE( StunEvilEnemyChance, 60 )" );
            main.addLineIndent( "    SET_GAME_RULE( StunGoodEnemyChance, 50 )" );
            main.addLineIndent( "ENDIF" );
            main.addLineIndent( "IF( PLAYER0,FLAG7 >= 4 )" );
            main.addLineIndent( "    SET_GAME_RULE( StunGoodEnemyChance, 30 )" );
            main.addLineIndent( "ENDIF" );
        }

        void Script::setSacrificeLimits() {
            main.addLineIndent( "rem - set sacrifice limits -" );
            main.addLineIndent( "IF( PLAYER0, SACRIFICED[IMP] >= 9 )" );
            main.addLineIndent( "    REMOVE_SACRIFICE_RECIPE( IMP )" );
            main.addLineIndent( "ENDIF" );
            main.addLineIndent( "IF( PLAYER0, REWARDED[BILE_DEMON] >= 1 )" );
            main.addLineIndent( "    REMOVE_SACRIFICE_RECIPE( SPIDER, SPIDER, SPIDER )" );
            main.addLineIndent( "ENDIF" );
            main.addLineIndent( "IF( PLAYER0, REWARDED[DARK_MISTRESS] >= 1 )" );
            main.addLineIndent( "    REMOVE_SACRIFICE_RECIPE( SPIDER, BUG )" );
            main.addLineIndent( "ENDIF" );
            main.addLineIndent( "IF( PLAYER0, REWARDED[TENTACLE] >= 1 )" );
            main.addLineIndent( "    REMOVE_SACRIFICE_RECIPE( SPIDER, TROLL )" );
            main.addLineIndent( "ENDIF" );
            main.addLineIndent( "IF( PLAYER0, REWARDED[HORNY] >= 1 )" );
            main.addLineIndent( "    REMOVE_SACRIFICE_RECIPE( TROLL, BILE_DEMON, DARK_MISTRESS )" );
            main.addLineIndent( "ENDIF" );
        }

        void Script::setGraveyardLimits() {
            main.addLineIndent( "rem - set graveyard limits -" );
            main.addLineIndent( "IF( PLAYER0, VAMPIRE >= 2 )" );
            main.addLineIndent( "    SET_GAME_RULE( BodiesForVampire, 15 )" );
            main.addLineIndent( "ENDIF" );
            main.addLineIndent( "IF( PLAYER0, VAMPIRE >= 4 )" );
            main.addLineIndent( "    SET_GAME_RULE( BodiesForVampire, 20 )" );
            main.addLineIndent( "ENDIF" );
            main.addLineIndent( "IF( PLAYER0, VAMPIRE >= 6 )" );
            main.addLineIndent( "    SET_GAME_RULE( BodiesForVampire, 25 )" );
            main.addLineIndent( "ENDIF" );
        }
        ...
        const ScriptCommand& Script::getSection( const ScriptSection section ) const {
            ...
            case ScriptSection::SS_FLAGS:   return flags;
        ...
        std::vector< std::string > Script::build() const {
            ...
            addSection( merged, flags,         "REM ============  FLAGS SETUP  ============" );
